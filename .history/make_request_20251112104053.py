...
{{
    ...
    "global_score": {{
        ...
        # Vérifier la structure de global_score
        global_score = enrichment_data.get("global_score", {})
        if "score" not in global_score:
            logger.error("❌ 'score' manquant dans global_score")
            raise ValueError("Score manquant dans la réponse")
        
        score_recu = global_score["score"]
        logger.info(f"   🏆 Score reçu de l'IA: {score_recu}/5")
        
        # Score de l'IA accepté sans correction forcée
        logger.info(f"✅ Score de l'IA accepté: {score_recu}/5")
...
async def analyze_complete_logement_parallel(input_data: EtapesAnalysisInput) -> CompleteAnalysisResponse:
    ...
    try:
        ...
        for piece_analysis in pieces_analysis_results:
            ...
            if not hasattr(piece_analysis, 'issues') or piece_analysis.issues is None:
