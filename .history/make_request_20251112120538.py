...
def build_dynamic_prompt(input_data: InputData) -> str:
    ...
    try:
        ...
        logger.info(f"🔧 Prompt pour {input_data.nom}: {len(input_data.elements_critiques)} critiques, {len(input_data.points_ignorables)} ignorables, {len(input_data.defauts_frequents)} défauts")
    ...
    try:
        ...
        try:
            ...
            if "preliminary_issues" not in response_json or not isinstance(response_json["preliminary_issues"], list):
                logger.warning("⚠️ preliminary_issues manquant ou invalide, correction appliquée")
...
class CombinedAnalysisResponse(BaseModel):
    ...
    analyse_globale: AnalyseGlobale
    issues: List[Probleme]  # Issues générales de la pièce
    etapes_analysis: List[Probleme] = []  # Issues des étapes (ajouté pour stocker les issues d'étapes)
...
def calculate_weighted_severity_score(
    ...
    total_issues_in_pieces = sum(len(p.issues) + len(p.etapes_analysis) for p in pieces_analysis)
    logger.info(f"   - {total_issues_in_pieces} issues TOTALES dans les objets pieces_analysis")
    for piece in pieces_analysis:
        logger.info(f"      Pièce {piece.piece_id}: {len(piece.issues)} générales + {len(piece.etapes_analysis)} étapes")
    ...
    for idx, piece in enumerate(pieces_analysis, 1):
        ...
        if hasattr(piece, 'issues') and piece.issues:
            for issue in piece.issues:
                if issue.confidence >= CONFIDENCE_THRESHOLD:
                    ...
                    # Pas de multiplicateur de pièce ici (déjà dans le poids)
        ...
        # Issues d'étapes (réduites selon le facteur configuré)
        # etapes_analysis est maintenant une liste de Probleme directement
        if hasattr(piece, 'etapes_analysis') and piece.etapes_analysis:
            for issue in piece.etapes_analysis:
                if issue.confidence >= CONFIDENCE_THRESHOLD:
                    base_score = SEVERITY_BASE_SCORE.get(issue.severity, 1)
                    category_mult = CATEGORY_MULTIPLIER.get(issue.category, 1.0)
                    ...
                    piece_issues_details.append({
                        "description": issue.description,  # La description contient déjà "[ÉTAPE]" si c'est une issue d'étape
                        "category": issue.category,
                        "severity": issue.severity,
                        "score": round(issue_score, 2),
                        "is_etape": True
...
async def analyze_single_piece_async(piece: PieceWithEtapes) -> CombinedAnalysisResponse:
    """
    ...
    """
    try:
        logger.info(f"🔍 [ASYNC] Analyse de la pièce {piece.piece_id}: {piece.nom}")
...
async def analyze_complete_logement_parallel(input_data: EtapesAnalysisInput) -> CompleteAnalysisResponse:
    ...
    """
    try:
        logger.info(f"🚀 [PARALLEL] ANALYSE COMPLÈTE démarrée pour le logement {input_data.logement_id}")
        ...
        # Créer les tâches pour toutes les pièces
        piece_tasks = [analyze_single_piece_async(piece) for piece in input_data.pieces]
        ...
        # Calcul des compteurs et reconstruction des objets avec etapes_analysis
        total_issues_count = 0
        general_issues_count = 0
        etapes_issues_count = len(all_etape_issues)

        # Reconstruire les objets CombinedAnalysisResponse avec les issues d'étapes
        ...
        for piece_analysis in pieces_analysis_results:
            ...
            # Créer un nouvel objet avec les issues d'étapes
            updated_piece_analysis = CombinedAnalysisResponse(
                ...
                issues=piece_analysis.issues,
                etapes_analysis=etapes_issues_for_piece
            )

            updated_pieces_analysis.append(updated_piece_analysis)

            # Compter le total (issues générales + issues d'étapes)
            total_issues_count += general_issues_for_piece + len(etapes_issues_for_piece)
        ...
        logger.info(f"🔍 DEBUG - Après reconstruction des objets:")
        for piece_result in pieces_analysis_results:
            logger.info(f"   Pièce {piece_result.piece_id}: {len(piece_result.issues)} issues générales + {len(piece_result.etapes_analysis)} issues d'étapes")
...
def save_room_templates(templates_data):
    ...
    global ROOM_TEMPLATES  # 🔥 AJOUT CRITIQUE
    ...
    # 🚨 CORRECTION CRITIQUE : Mettre à jour la variable globale en mémoire
    if success_local or success_env:
        ROOM_TEMPLATES = templates_data
        logger.info("🔥 ROOM_TEMPLATES mis à jour en mémoire - Modifications IMMÉDIATEMENT effectives!")
...
def load_prompts_config():
    """Charger la configuration des prompts depuis le fichier JSON"""
    try:
        # 🔥 PRIORITÉ 1: Variable d'environnement Railway (production)
        prompts_config_env = os.environ.get('PROMPTS_CONFIG')
        if prompts_config_env:
            try:
                logger.info("📡 Chargement de la config prompts depuis les variables d'environnement Railway")
                return json.loads(prompts_config_env)
            except json.JSONDecodeError as e:
                logger.error(f"❌ Erreur lors du parsing JSON de PROMPTS_CONFIG: {e}")
        
        # 🔥 PRIORITÉ 2: Fichier local (développement/fallback)
        possible_paths = [
            "front/prompts-config.json",
            "prompts-config.json",
            os.path.join(os.path.dirname(__file__), "front", "prompts-config.json")
        ]
        ...
        # 🔥 PRIORITÉ 3: Configuration par défaut
        logger.warning("⚠️ Aucune config prompts trouvée, utilisation de la configuration par défaut")
...
def save_prompts_config(config_data):
    """Sauvegarder la configuration des prompts"""
    success_local = False
    success_env = False
    ...
    try:
        ...
        if not target_path:
            ...
            target_path = "front/prompts-config.json"
        
        with open(target_path, 'w', encoding='utf-8') as f:
            json.dump(config_data, f, indent=2, ensure_ascii=False)
        
        logger.info(f"✅ Config prompts sauvegardée dans le fichier: {target_path}")
        success_local = True
        
    except Exception as e:
        logger.error(f"❌ Erreur sauvegarde fichier local prompts: {e}")
    ...
    try:
        ...
        # Note: En production Railway, cette mise à jour nécessitera un redémarrage
        os.environ['PROMPTS_CONFIG'] = config_json
        
        logger.info("✅ Config prompts mise à jour dans les variables d'environnement")
        success_env = True
        
        # 🔥 IMPORTANT: Informer l'utilisateur pour Railway
        if os.environ.get('RAILWAY_ENVIRONMENT'):
            logger.warning("⚠️ RAILWAY: Les modifications seront perdues au prochain déploiement!")
            logger.warning("💡 Utilisez l'interface d'admin Railway pour définir PROMPTS_CONFIG de façon permanente")
        
    except Exception as e:
        logger.error(f"❌ Erreur sauvegarde variable d'environnement prompts: {e}")
...
async def get_prompts_config():
    """Récupérer la configuration complète des prompts"""
    try:
        ...
        logger.error(f"Erreur lors de la récupération de la config prompts: {e}")
        raise HTTPException(status_code=500, detail=str(e))
