...
class PromptManager {
    ...
    async loadPrompts() {
        try {
            ...
            if (apiResponse.success && apiResponse.config) {
                ...
                this.showNotification('Configuration des prompts chargée avec succès', 'success');
            } else {
    ...
    async saveAll() {
        try {
            ...
            if (result.success) {
                ...
                this.showNotification(`✅ Configuration sauvegardée avec succès (${result.last_updated})`, 'success');
            } else {
    ...
    exportJson() {
        try {
            ...
            link.download = `prompts-config-${new Date().toISOString().split('T')[0]}.json`;
